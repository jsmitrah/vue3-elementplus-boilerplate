var Vue = (function (exports) {
  ...
  */
  ...
  const setRef = (rawRef, oldRawRef, parentSuspense, vnode) => {
    if (isArray(rawRef)) {
      rawRef.forEach((r, i) =>
        setRef(r, oldRawRef && (isArray(oldRawRef) ? oldRawRef[i] : oldRawRef), parentSuspense, vnode)
      );
      return;
    }
    let value;
      ...
      if (prop.type === 6 /* ATTRIBUTE */) {
        ...
        if (!arg && (isBind || isOn)) {
          ...
          if (exp) {
            ...
            context.onError(
              createCompilerError(isBind ? 33 /* X_V_BIND_NO_EXPRESSION */ : 34 /* X_V_ON_NO_EXPRESSION */, loc)
            );
          }
          continue;
        }
    ...
    if (
      ...
      return () => {
        ...
        if (
          ...
          (children.length === 1 &&
            (node.type === 0 /* ROOT */ || (node.type === 1 /* ELEMENT */ && node.tagType === 0)) /* ELEMENT */)
        ) {
          return;
        }
